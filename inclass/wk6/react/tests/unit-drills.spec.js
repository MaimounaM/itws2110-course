// Instructor check for drills 5–8. No browser: it shells out to Vitest twice and
...
test('worked answers for drills 5-8 pass', () => {
  const { code, output } = runVitest('test:unit:answers');
  expect(output).toContain('4 passed');
  expect(code, `vitest answers run failed:\n${output}`).toBe(0);
});

test('drill 5-8 starters begin red, and fail on the answer rather than on the setup', () => {
  ...
  for (const title of [
    '5. the heading says Pantry',
    '6. a product shows its name and its freshness',
    '7. clicking twice shows Added 2',
    '8. typing a new name updates the paragraph',
  ]) {
