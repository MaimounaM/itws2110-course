// Worked answers. Compare after attempting each drill.
...
export function Drill3() {
  return <div className="flex items-center justify-between"><strong>Pantry</strong><span>3 items</span></div>;
}
...
export function Drill5() {
  ...
  return <ul className="grid grid-cols-1 gap-4 md:grid-cols-3">
    {products.map(product => <li key={product.id} className="rounded bg-slate-100 p-4">{product.name}</li>)}
  </ul>;
}

// 6. A condition chooses text. Use expired ? 'Expired' : 'Fresh'.
function Freshness({ expired }) {
  return <span className="rounded bg-amber-100 px-2 py-1">{expired ? 'Expired' : 'Fresh'}</span>;
}
...
export function Drill8() {
  return <div className="flex gap-4"><Counter /><Counter /></div>;
}
...
function SharedCounter({ count, onAdd }) {
  return <button className="rounded border px-4 py-2" onClick={onAdd}>Count: {count}</button>;
}
...
export function Drill10() {
  ...
  return <div className="space-y-3">
    ...
    <input id="product-name" className="rounded border p-2" value={name} onChange={e => setName(e.target.value)} />
